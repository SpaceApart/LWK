 ...
 * This generates additional dashboard-specific types based on Payload collections
 */

import { readFileSync, writeFileSync, existsSync } from 'fs';
import { join } from 'path';

const PAYLOAD_TYPES_PATH = join(__dirname, '../src/payload-types.ts');
const DASHBOARD_TYPES_TEMPLATE = join(__dirname, '../templates/dashboard-types.template.ts');
const DASHBOARD_TYPES_OUTPUT = join(__dirname, '../src/dashboard-types.ts');
...
function generateDashboardTypes() {
  ...
  if (!existsSync(PAYLOAD_TYPES_PATH)) {
    console.error('❌ Payload types not found. Run generate:types in admin app first.');
    process.exit(1);
  }

  // Read payload types
  const payloadTypes = readFileSync(PAYLOAD_TYPES_PATH, 'utf-8');

  // Extract collection names
  const collectionMatches = payloadTypes.matchAll(/export interface (\w+) \{/g);
  const collections = Array.from(collectionMatches).map(match => match[1]);
...
`;
  ...
  writeFileSync(DASHBOARD_TYPES_OUTPUT, dashboardTypes);
